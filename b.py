...
for priceIndex in range(len(Jtable)):
    if (not(priceIndex > (X0 + N) or priceIndex < (X0 - N))) and priceIndex <= priceToSell:
...
for colIndex in range(N-1, -1, -1):
    ...
    for priceIndex in range(priceToSell + 1):
        ...
        if priceIndex > (X0 + colIndex) or priceIndex < (X0 - colIndex):
            ...
            if priceIndex == 0:
                ...
                if priceIndex == priceToSell:
                    Jtable[priceIndex][colIndex] = priceToSell
                else:
                    #print(colIndex)
                    #print(priceIndex)
                    pnoChange = 1 - (2* p)
